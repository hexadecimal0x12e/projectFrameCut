...
{
    [TestMethod]
    public async Task AdditionalPipesShareBackendReconnectAndStopWithHost()
    {
        ...
        try
        {
            ...
            await using var reconnected = Connect(RenderProtocol.AdditionalPipePrefix + tokens[0], tokens[0]);
            Assert.AreEqual("shared-instance", (await reconnected.GetCapabilitiesAsync(lifetime.Token)).BackendVersion);
        ...
        {
            using var pipe = new NamedPipeClientStream(".", RenderProtocol.AdditionalPipePrefix + token, PipeDirection.InOut, PipeOptions.Asynchronous);
