...
public partial class DebuggingMainPage : ContentPage
{
    ...
    string draftSourcePath
     ...
     = @"D:\code\playground\projectFrameCut\My Drafts\test";
...
draftSourcePath = "";
    ...
    private async void ToV2DraftPageWithSomeClips_Clicked(object sender, EventArgs e)
    {
        ...
        page = new DraftPage(new(),dict, DraftImportAndExportHelper.ImportAssetsFromJSON(assetJson), trackCount, "1");

        await Navigation.PushAsync(page);

    }

    private async void ToV2DraftPageWithReadonly_Clicked(object sender, EventArgs e)
    {
        try
        {
            ...
            page = new DraftPage(project ?? new(), dict, assetDict, trackCount, draftSourcePath, project?.projectName ?? "?",true);
#if WINDOWS
            App.HideNavBar();
#endif
            await Navigation.PushAsync(page);
